...
- **Dune account**: any free Dune account (the SQL is self-contained)
- **Query ID**: `6912877` (self-contained — no external materialized view dependency)
- **Extraction date**: March 2026
- **Query liveness last verified**: June 2026 (query `6912877` resolves and returns rows)

The query can be re-executed directly from the Dune Analytics interface or via the API (`dune.get_latest_result(6912877)`); the result must be saved to `data/raw/defi/defi_liquidations_1h_clean.csv`. Results may differ marginally from future executions if Spellbook is updated. The SQL source is also provided in the `dune_queries/` directory of this repository.
...
This window is identical to the master calendar produced by `run_data_prep.py`.
...
### Derived variables (computed in `run_defi_merge.py`)
...
Additional cross-checks against May 2021, Terra/Luna (June 2022), FTX (November 2022), and DeepSeek (February 2025) confirm that liquidation spikes align with independently documented stress dates. See `04_defi_merge_report.ipynb`, section "Known Event Cross-Check."
...
This file is delivered in the raw/normalized bundle (GitHub Release asset), not committed. Place it at `data/raw/defi/defi_liquidations_1h_clean.csv` (the path `run_defi_merge.py` reads directly). Required only for the full rebuild (`make data`/`make all`); the shipped panel already incorporates it.
...
- Hours with zero liquidations are absent; they are filled with zeros via LEFT JOIN on the master calendar in `run_defi_merge.py`
