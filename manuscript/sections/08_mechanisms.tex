...
plausibly lives (Section~\ref{sec:discussion}) is unlikely, at the observable
scale, to be a material aggregate contributor to ETH's crash quantile. This
size argument is conditional on observed coverage: the MakerDAO and
liquid-restaking collateral that Section~\ref{sec:data-limits} documents as
outside the measure do not enter the numerator, and the omission is bounded
there as favorable to the null. In
...
\label{tab:sizeratio}
\input{../paper/tables/tab5_size_ratio}
...
is our own hard extraction; the caveat is detailed at the table). Because the
numerator is debt forcibly repaid rather than collateral seized and sold, it
excludes the liquidator's discount and so understates the volume that
actually reaches the market, which makes the ratio a conservative bound on
aggregate materiality. The flow
