...
Section~\ref{sec:apparent} yet statistically the least informative. Volatility
...
becoming ETH's return, by symmetry; BTC has no direct ETH-collateral
forced-selling channel, yet is a highly correlated benchmark, so a
replicated signature demonstrates non-specificity rather than a null
effect). The ETH signature replicates on Bitcoin in every respect
...
bias. In \emph{shape}, the raw QLP asymmetry is not evidence of economically meaningful downside-specific shape. A zero-skew world
