...
\begin{equation}
\label{eq:qlp}
\QR_{\tau}\!\left(r_{t \to t+h} \mid \mathcal{F}_t\right)
 = \beta_h(\tau)\, L_{t-1}
 + \delta_h(\tau)\, \left(L_{t-1} \times \texttt{oi\_high}_t\right)
 + \gamma_h(\tau)' X_t ,
\end{equation}
where $r_{t \to t+h}$ is the cumulative ETH return from $t$ to $t+h$,
...
estimator and its bootstrap inference follow \citet{han2024}.
...
quantile grid. The direct shock effect survives both families in full
