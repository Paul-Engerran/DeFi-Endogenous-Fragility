...
leverage effect. In crypto, that channel is often found to be weak, absent, or inverted
\citep{baur2018,brini2022,aharon2023}, and a commonly invoked alternative
mechanism is the liquidation cascade of the leverage cycle
...
financial-stability assessments \citep{oecd2023,fsb2023}. In centralized venues the full mechanism is largely opaque.
...
best-1\% hours and \PLiqAll\% unconditionally. Two coverage gaps further bound the claim: MakerDAO, historically among the largest liquidation venues, is de facto absent from the panel, and the sample predates the liquid-restaking-token collateral generation; we assess the direction of both in Section~\ref{sec:discussion}. The register of the paper is
...
predicts, and what this nascent literature could plausibly report before
...
endogenous fragility. It is not. Showing why, with a
...
Net of volatility, we can bound two objects. The one-percent-tail
exceedance asymmetry (down minus up) is below our pre-specified smallest
effect size of interest under all three calibrations of the shock span,
including the strictest; a small standardized residual is statistically
detectable but economically negligible, clearing two of the three
calibrations and borderline under the strictest. Jointly they exclude
downside-specific amplification larger than \MdeEightyExc{} per unit of the
log-liquidation shock. This bound is a
...
the apparatus is worth porting is itself a finding. It survives the
adversarial repairs we put to it, and its positive control confirms the
...
\MdeEightyExc{} per unit of shock at the one-percent tail (the exceedance
asymmetry clears all three calibrations, including the strictest; the small
standardized residual clears two and is borderline under the strictest), a
falsifiable statement rather than a bare failure to reject. The whole is made reproducible by an hourly
