...
v2--v3, Spark, and Morpho account for the bulk of volume). The query was
executed once, and the deposited hourly panel, not a live re-query, is the
reproducibility anchor; the source-to-panel path is therefore a snapshot and
is not end-to-end re-executable. The primary
...
thousand (both computed over all calendar days, not only active days); and the largest single day (\$\MaxDayM{} million, 2024-08-05, the
...
\input{../paper/tables/tab1_descriptives}
